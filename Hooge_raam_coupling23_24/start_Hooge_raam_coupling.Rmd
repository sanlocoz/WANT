...
plot(c(-rev(widthrange/2),widthrange/2),c(rev(arange),arange),
     xlab="y (m)",ylab="a (m)",main="cross section",type="l",lwd=5,col="brown")
grid(col="black")
```
...
plot(Qrange,arange,
     ...
     xlab=expression(Q~(m^3/s)),ylab="a (m)",col="brown",lwd=3,type="l")
...
plot(Qequirange,arange,
     ...
     xlab=expression(Qequi~(m^3/s)),ylab="a (m)",lwd=3,col="blue",type="l")
grid(col="black")
```
...
plot(Qequirange,arange,main="Qequi (without weir) and Qweir for HoogeRaam",
     xlab=expression(Qequi~(m^3/s)),ylab="a (m)",lwd=3,col="blue",type="l", ylim = c(0,1.5))
...
```{r}
Pattern.maximum.rate = 0.015*24
Pattern.storm.length = 24 #hours

P.pattern.rate = c(0.0008,Pattern.maximum.rate*(sin(pi*rep(1/24*(1:Pattern.storm.length)))),0.0008)
P.pattern.date = c(0,rep(1/24*(1:Pattern.storm.length)),((Pattern.storm.length+1)/24))
P.pattern = approxfun(P.pattern.date,P.pattern.rate,rule = 2)
plot(P.pattern.date,P.pattern(P.pattern.date),type="l",col = "blue",
     lwd = 2, xlab = "days",ylab = "intensity in m/d",main = "Storm event in one day")
grid()
```
...
while (current.time < end.time)
{
  ...
  P = P.pattern(current.time) #precipitation during the storm 
...
m2.day.to.m3.day =   HR.discharge.area/L #from m2/day to m3/day; a "diffuse length" based on the total drained area divided by the L(ditch distance); unit is m because it is used to convert m2/day to m3/day
...
```{r eval= TRUE, animation1,  fig.show='animate', aniopts=c('controls','loop'), interval=0.1}
...
MRESULT = function(n)
{
  if(n == 1)#Maximum discharge of the Hooge Raam at the weir
  {
    max.discharge= max(HR.Q.trans$boundary_downstream)
    return(max.discharge)
  }
  else if(n == 2)#Max head of groundwater
  {
    max.head= max(GW.H.trans)
    return(max.head)
  }
  else if(n == 3)#Max water height at Hooge Raam
  {
    max.height= max(HR.h.trans$X50)
    return(max.height)
  }
  else if(n == 4) #Max drainage of the groundwater
  {
    max.drain.HR= max(GW.Q.trans$boundary)
    return(max.drain.HR)
  }
  else if(n == 5) #Lag of maximum precipitation and the maximum head of groundwater
  {
    maximum_head = max(GW.H.trans)
    eps = 10^-7
    index = c(0,0)
    dimension = dim(GW.H.trans)
    for(i in (1:dim(GW.H.trans)[2]))
    {
      for(j in (1:dim(GW.H.trans)[1]))
      {
        if(abs(maximum_head - GW.H.trans[j,i])<eps) #compare if GW.H.trans now == what we search
        {
          index = c(i,j) # i is column, j is row, if it is equal take the index
        }
      }
    }
    
    maximum_precipitation = max(GW.Q.trans$precip)
    indexp = c(0,0)
    for(i in (1:length(GW.Q.trans$precip)))
    {
        if(abs(maximum_precipitation - GW.Q.trans$precip[i])<eps) #compare if GW.Q.trans now == what we search
        {
          indexp = c(i) # if it is equal take the index
        }
    }
    
    return((index[2]-indexp)*delt.t)
  }
}
...
```{r eval = TRUE}
base = list(GW.kD = GW.kD, C_ditch = C_ditch, H_drainage = H_drainage, C_surface = C_surface, H_surface = H_surface, GW.S = GW.S, HR.weir.crest = HR.weir.crest, HR.weir.width = HR.weir.width, HR.n = HR.n, HR.b = HR.b, HR.m = HR.m, Pattern.maximum.rate = Pattern.maximum.rate, Pattern.storm.length = Pattern.storm.length) # the list with the current model parameters
...
```{r eval = TRUE}
scale = list(GW.kD = 0.5*GW.kD, C_ditch = 0.6*C_ditch, H_drainage = 0.3, C_surface = 0.6*C_surface, H_surface = 0.3, GW.S = 0.4*GW.S, HR.weir.crest = 0.10, HR.weir.width = 0.05, HR.n = 0.25*HR.n, HR.b = 0.3, HR.m = 0.2*HR.m, Pattern.maximum.rate = 0.1*Pattern.maximum.rate, Pattern.storm.length = 1/24)#list of the scale of variation of the model parameters
...
```{r eval = TRUE} 
# Output of interest:
#   1. Maximum discharge at the weir (MRESULT1)
#   2. Maximum head of groundwater (MRESULT2)
#   3. Maximum water height at Hooge Raam (MRESULT3)
#   4. Maximum drainage of the groundwater (MERSULT4)
#   5. Lag of maximum precipitation and the maximum head of groundwater or with max runoff (MRESULT5)
...
# 
# open water model:
# 1. Weir crest height HR.weir.crest
# 2. Weir width HR.weir.width
# 3. Manning coefficient HR.n
# 4. River width HR.b
# 5. Side slopes HR.m
# 
# external fluxes:
# 1. Maximum storm rate Pattern.maximum.rate
# 2. Length of the storm Pattern.storm.length: num 24
...
{
  print(paste("Still working at", i, "/",length(base)))
  base.current = as.numeric(base[i]) #base value of parameter[i]
  scale.current = as.numeric(scale[i]) #scale of variation in terms of standard deviation of parameter[i]
  eps = scale.current*0.01 #we take epsilon as 1% of the scale of variation (standard deviation)
  
  name = names[i] #variables that we want to change
  
  assign(name, base.current+eps, envir = .GlobalEnv) #assigning name to base+eps

  Sensitivity.loop() #running the coupling model
  
  assign(name, base.current, envir = .GlobalEnv) #changing variables back to base
  
  ans <- c() #The size MRESULT, how many variables of interest we have
  for(j in (1:5))
  {
    slope = (MRESULT(j)-M_base[j])/eps
    sd = slope*scale.current
    var = sd^2
    
    ans = c(ans, var)
  
  }
  varRes = rbind(varRes, ans)
}
colnames(varRes) <- c('MRESULT1', 'MRESULT2', 'MRESULT3', 'MRESULT4', 'MRESULT5')
relVarM = sweep(varRes,2,colSums(varRes),`/`)

varRes = cbind(names, varRes)
relVarM = cbind(names, relVarM)
```
</div>
...
# Output of interest:
#   1. Maximum discharge at the weir (MRESULT1), m3/s
#   2. Maximum head of groundwater (MRESULT2), m
#   3. Maximum water height at Hooge Raam (MRESULT3), m
#   4. Maximum drainage of the groundwater (MERSULT4), m2/d
#   5. Lag of maximum precipitation and the maximum head of groundwater or with max runoff (MRESULT5), day

nameResult = c("MRESULT1","MRESULT2","MRESULT3","MRESULT4","MRESULT5")

varMtot = c() #Total Variance of MRESULT1 - MRESULT5
for(i in (2:6))
{
  varMtot = c(varMtot, sum(varRes[,i]))
}

knitr::kable(as.data.frame(relVarM), caption = "Ratio of relative variances",align="c")
knitr::kable(as.data.frame(varRes), caption = "Variance per parameter for each MRESULT",align="c")
knitr::kable(cbind(nameResult,as.data.frame(M_base)), caption = "MBASE",align="c")
knitr::kable(cbind(nameResult,as.data.frame(varMtot)), caption = "Total Variance",align="c")
...
for(i in (1:4))
{
  ...
  pie(data,labels=labels,
    main=paste("Standard deviation of M",i,"=",round(sqrt(varMtot[i]),4), unit(i)),
